...
import { decodeKaggleWorldCupMatches } from "./loadKaggleWorldCupMatches.ts";
import type {
...
export interface ValidatedHistoricalDataset {
  summary: HistoricalDatasetValidationSummary;
  normalizedMatches: NormalizedHistoricalMatch[];
}

export interface HistoricalDatasetLoadDependencies {
  readSourceFile?: (filePath: string) => Promise<Uint8Array>;
}
...
export async function validateHistoricalDataset(input: {
  ...
  return (await loadValidatedHistoricalDataset(input)).summary;
}

export async function loadValidatedHistoricalDataset(input: {
  sourceFile?: string;
  provenanceFile?: string;
} = {}, dependencies: HistoricalDatasetLoadDependencies = {}): Promise<ValidatedHistoricalDataset> {
  ...
  const sourceBuffer = await (dependencies.readSourceFile ?? readFile)(
    resolve(sourceFile),
  );
  ...
  if (sourceBuffer.byteLength !== provenance.fileSizeBytes) {
    throw new Error(
      `Historical source size ${sourceBuffer.byteLength} does not match provenance size ${provenance.fileSizeBytes}.`,
    );
  }
  ...
  const sourceRows = decodeKaggleWorldCupMatches(sourceBuffer, sourceFile);
  ...
  return {
    ...
    normalizedMatches: normalized.records.map(
      (record) => record.normalizedMatch,
    ),
  };
}
