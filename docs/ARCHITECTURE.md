...
snapshot scripts. UI wiring is deferred until the generated official knockout
bracket is deliberately wired into the app. The current official snapshot is a
complete 72-match group-stage results record and now resolves to
`knockout_ready`.

The fair-play source-gap verifier is also offline and script-driven. It checks
the reviewed source-gap artifact and confirms Ecuador/Ghana fair-play totals
remain missing rather than zero. Ecuador and Ghana share third-place rank 3 on
available criteria; both qualify, and their unresolved strict ordering does not
affect the qualifying third-place group key `BDEFIJKL`, Annex C assignment, or
Round of 32. The verifier uses a fixed seven-candidate manifest with stable
candidate IDs, structured source outcomes, bounded-conclusion checks, and direct
filesystem checks for the finalized qualification, Round-of-32, rating-linkage,
and simulator-input artifacts. Those artifacts are generated by explicit
Node-only scripts and are not imported by React/UI code yet.
